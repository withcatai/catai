...
import { useBuiltInAPISettings } from '../../hooks/useBuiltInAPISettings';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const { settings: apiSettings, setSetting: setAPISetting, resetSettings: resetAPISettings } = useBuiltInAPISettings();
  ...
  const handleAPISettingChange = async (key: keyof typeof apiSettings, value: boolean) => {
    setAPISetting(key, value);

    // Auto-restart server when API settings change
    try {
      ...
      addToast('Error restarting server', 'error');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        {activeTab === 'api' && (
          <>
            <div>
              ...
              {/* Web Search Toggle */}
              <div className="flex items-center justify-between p-3 mb-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                <div>
